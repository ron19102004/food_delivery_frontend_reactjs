...
const EditCategoryForm: React.FC<IEditCateForm> = ({
  ...
  return (
    <div className="px-6">
      ...
      <div className="mb-2">
          <label className="block text-lg font-font2">Image url from google driver</label>
          <p className="text-sm">Require turn on general access to anyone on the internet with the link can view</p>
          <Input
            {...register("img_url", {
              required: {
                value: true,
            ...
            required
          />
          {errors.img_url && <p className="text-red-600">{errors.img_url.message}</p>}
        </div>
        <div className="mb-2">
